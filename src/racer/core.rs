...
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq, Ord, PartialOrd, Index, From,
         Add, Sub, AddAssign, SubAssign)]
pub struct BytePoint(pub usize);

impl From<u32> for BytePoint {
    fn from(u: u32) -> Self {
        BytePoint(u as usize)
    }
}
...
impl ByteRange {
    pub fn new<P: Into<BytePoint>>(start: P, end: P) -> Self {
        ByteRange {
            start: start.into(),
            end: end.into(),
        }
    ...
    pub fn contains(&self, point: BytePoint) -> bool {
        self.start <= point && point < self.end
    }
    pub fn shift<P: Into<BytePoint>>(&self, shift: P) -> Self {
        ByteRange {
    ...
    pub fn to_tuple(&self) -> (BytePoint, BytePoint) {
        (self.start, self.end)
    }
}
...
impl From<BytePoint> for Location {
    fn from(val: BytePoint) -> Location {
        Location::Point(val)
    }
}
...
pub trait LocationExt {
    fn to_point(&self, src: &IndexedSource) -> Option<BytePoint>;
...
impl IndexedSource {
    ...
    pub fn as_src(&self) -> Src {
        self.src_from_start(BytePoint::zero())
    }

    pub fn src_from_start(&self, new_start: BytePoint) -> Src {
        Src {
            src: self,
    ...
    pub fn coords_to_point(&self, coords: &Coordinate) -> Option<BytePoint> {
    ...
    pub fn point_to_coords(&self, point: BytePoint) -> Option<Coordinate> {
        self.cache_lineoffsets();
...
impl<'c> Src<'c> {
    pub fn start(&self) -> BytePoint {
        self.range.start
    }

    pub fn end(&self) -> BytePoint {
        self.range.end
    }
    ...
    pub fn shift_start(&self, shift: BytePoint) -> Src<'c> {
        Src {
            src: self.src,
    ...
    pub fn change_length(&self, new_length: BytePoint) -> Src<'c> {
        Src {
            src: self.src,
