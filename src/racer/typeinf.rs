...
use core::{Match, Src, Scope, Session, SessionExt, BytePoint};
    ...
    match blob.find('(') {
        ...
        Some(probable_param_start) => {
            ...
            if let Some(start) = blob[skip_generic..].find('(') {
                let start = BytePoint::from(start).increment();
...
pub fn get_type_of_self(
    ...
    scopes::find_impl_start(msrc, point, BytePoint::zero()).and_then(|start| {
...
fn find_start_of_closure_body(src: &str) -> Option<BytePoint> {
    let mut cnt = 0;
    for (i, c) in src.chars().enumerate() {
        ...
        if cnt == 2 {
            return Some(BytePoint::from(i).increment());
        }
    }
    warn!(
...
fn get_type_of_let_block_expr(m: &Match, msrc: Src, session: &Session, prefix: &str) -> Option<core::Ty> {
    ...
    let point: BytePoint = stmt.find(prefix).expect("`prefix` should appear in statement").into();
...
fn get_type_of_for_expr(m: &Match, msrc: Src, session: &Session) -> Option<core::Ty> {
    ...
    if let Some(range) = src.as_src().iter_stmts().next() {
        ...
        let pos = m.point + BytePoint::from(8) - stmtstart - forpos.into() - range.start;
        let scope = Scope{ filepath: m.filepath.clone(), point: stmtstart };
...
pub fn get_tuplestruct_field_type(fieldnum: usize, structmatch: &Match, session: &Session) -> Option<core::Ty> {
    ...
    let structsrc = if let core::MatchType::EnumVariant(_) = structmatch.mtype {
        ...
        let to = src[structmatch.point.0..].find('(')
            .map(|n| scopes::find_closing_paren(&src, structmatch.point + BytePoint::from(n).increment()))
...
pub fn get_type_from_match_arm(m: &Match, msrc: Src, session: &Session) -> Option<core::Ty> {
    ...
    let faux_prefix_size = BytePoint::from(fauxmatchstmt.len());
