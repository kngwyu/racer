use core::{self, Match, MatchType, Scope, Ty, Session, SessionExt, BytePoint, ByteRange};
use typeinf;
...
fn point_is_in_span(point: BytePoint, span: &Span) -> bool {
...
struct LetTypeVisitor<'c: 's, 's> {
    ...
    session: &'s Session<'c>,
    srctxt: String,
    pos: BytePoint,        // pos is relative to the srctxt, scope is global
    result: Option<Ty>
}
...
fn find_type_match(path: &core::Path, fpath: &Path, pos: BytePoint, session: &Session) -> Option<Ty> {
...
struct StructVisitor {
    pub scope: Scope,
    pub fields: Vec<(String, BytePoint, Option<core::Ty>)>
}
            ...
            .filter_map(|bound| {
                if let TyParamBound::TraitTyParamBound(ref ptrait_ref, _) = *bound {
                    ...
                    let path_search = core::PathSearch {
                        ...
                        point: BytePoint::from((point as i32 + offset) as u32),
                    };
                    Some(path_search)
                } else {
                    None
...
pub struct EnumVisitor {
    pub name: String,
    pub values: Vec<(String, BytePoint)>
}
...
pub fn parse_struct_fields(s: String, scope: Scope) -> Vec<(String, BytePoint, Option<core::Ty>)> {
...
pub fn get_type_of(s: String, fpath: &Path, pos: BytePoint, session: &Session) -> Option<Ty> {
...
pub fn get_let_type(s: String, pos: BytePoint, scope: Scope, session: &Session) -> Option<Ty> {
...
pub fn get_match_arm_type(s: String, pos: BytePoint, scope: Scope, session: &Session) -> Option<Ty> {
