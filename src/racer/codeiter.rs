...
fn is_a_use_stmt(src_bytes: &[u8], start: BytePoint, pos: BytePoint) -> bool {
    let whitespace = b" {\t\r\n";
...
fn is_a_let_stmt(src_bytes: &[u8], start: BytePoint, pos: BytePoint) -> bool {
    let whitespace = b" {\t\r\n";
    ...
    where I: Iterator<Item = ByteRange>
{
    pub fn from_parts(src: &str, it: I) -> Fuse<StmtIndicesIter<I>> {
        StmtIndicesIter{ src, it, pos: BytePoint::zero(), end: BytePoint::zero() }.fuse()
    }
}
